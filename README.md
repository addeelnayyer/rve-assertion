...
> validator has its entry point, its structural phase and its structural
> signature phase; its semantic phase — validity window, audience, required
> attributes, identity cross-check — is not written yet, and no signature is
> cryptographically verified unless the caller supplies a verifier. **Do not
> spend an assertion on the strength of `validateAssertion` returning valid in
> this build.** See [Validating an assertion](#validating-an-assertion).
...
subject, and the conditions carrying the validity window. The signature phase
asks whether the signature covers this assertion. Each reports **one** failure
and runs nothing further. The semantic phase is the one that runs to completion
...
Four refusals are stricter than the specification demands, each argued in
